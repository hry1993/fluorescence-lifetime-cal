...
t = 1000 * t / (z - 1)
l1 = 15 # modify life 1 here
l2 = 170 # modify life 2 here
print(t)
...
for i in range(len(positions)):
    ...
    try:
        ...
        bV[l][m] = popt[1]
        dV[l][m] = popt[2]
        result[l][m][0] = (popt[0] * l1 * l1 + popt[3] * l2 * l2) / (popt[0] * l1 + popt[3] * l2)
        tValue[l][m] = result[l][m][0]
        allV[l][m][4] = tValue[l][m]
        a12V[l][m] = popt[0]/popt[3]
        resultD[l][m][0] = a12V[l][m]
        allV[l][m][5] = a12V[l][m]
        residuals = []
        ss_res = 0
        for k in range(z):
            residuals = y[k] - func(x[k], popt[0], popt[1], popt[2], popt[3])
            ss_res = ss_res + residuals ** 2
        ss_tot = np.sum((y - np.mean(y)) ** 2)
        result[l][m][1] = 1 - (ss_res / ss_tot)
        resultD[l][m][1] = result[l][m][1]
        rValue[l][m] = result[l][m][1]
        allV[l][m][6] = rValue[l][m]
        print(result[l][m])
    except:
        pass
